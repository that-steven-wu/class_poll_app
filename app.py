...
def submit():
    ...
    with FileLock(LOCK_PATH, timeout=10):
        ...
        for q in QUESTION_KEYS:
            for i, m in enumerate(METHOD_LABELS, 1):
                try:
                    fv = float(request.form.get(f"{q}_answer_{i}"))
                except:
                    fv = None
                rows.append({'question':q,'method':m,'answer':fv,'ts':now_utc})
        df_num = pd.concat([df_num, pd.DataFrame(rows)], ignore_index=True)
...
def results():
    ...
    for i, q in enumerate(keys):
        ...
        row_max = 1
        for m in METHOD_LABELS:
            vals = df[(df.question==q)&(df.method==m)]['answer']\
                   .dropna().loc[lambda s:(s>=lower)&(s<=upper)]
            if not vals.empty:
                cnts, _ = np.histogram(vals, bins=bins)
                row_max = max(row_max, cnts.max())

        for j, m in enumerate(METHOD_LABELS):
            ax = axes[i][j]
            subset = df[(df.question==q)&(df.method==m)]['answer']
            data = subset.dropna().loc[lambda s:(s>=lower)&(s<=upper)]
            all_vals = subset.dropna()

            nonzero = all_vals.loc[lambda s: s != 0]
            mean_val = nonzero.mean() if not nonzero.empty else np.nan
            sd_val = nonzero.std() if not nonzero.empty else np.nan

            if not data.empty:
                ax.hist(data, bins=bins, edgecolor='black', alpha=0.7)

            if not np.isnan(mean_val):
                offset = 0.10*(upper-lower)
                extra = offset/2
                x_anchor = mean_val if lower<=mean_val<=upper else (lower-extra if mean_val<lower else upper+extra)
                ax.axvline(x_anchor, color='blue', linestyle='-', linewidth=1.8, label='Mean')
                margin = 0.05*(upper-lower)
                mid = (lower+upper)/2
                y_pos = row_max*0.7
                if x_anchor<mid:
                    text_x = x_anchor+margin; ha='left'
                else:
                    text_x = x_anchor-margin; ha='right'
                ax.annotate(
                    '', xy=(x_anchor,y_pos), xytext=(text_x,y_pos),
                    arrowprops=dict(arrowstyle='-|>', color='blue', lw=1, shrinkA=0, shrinkB=0)
                )
                ax.text(text_x, y_pos, 'Mean', color='blue', ha=ha, va='center', fontsize=8, fontweight='bold')

            ax.axvline(corr, color='black', linestyle='--', linewidth=1.8, alpha=0.8, label='Correct')
            ax.set_xlim(lower-offset, upper+offset)
            ax.set_ylim(0, row_max*1.1)
            ax.set_xticks([lower,corr,upper])
            ax.xaxis.set_major_formatter(mtick.FuncFormatter(lambda x,pos:f"{x:,.1f}"))
            ax.grid(True,axis='y',linestyle='--',alpha=0.6)
            ax.set_title(f"{q} - {m}",fontsize=13)
            if j==0: ax.set_ylabel('Count',fontsize=11)
            if i==n_q-1: ax.set_xlabel('Answer',fontsize=11)
            ax.tick_params(axis='x',labelsize=9)
            ax.legend(fontsize=8)
            ax.text(0.5,-0.22,f"Mean: {mean_val:,.1f}   SD: {sd_val:,.1f}   Correct: {corr:,.1f}", transform=ax.transAxes, ha='center', va='top', fontsize=10, fontweight='bold')
